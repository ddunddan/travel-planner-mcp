...
⚠️ 필수 출력 규칙 (매번 반드시 지켜주세요 - 예외 없음)
1. Tool 결과를 절대 요약하지 마세요 - 전체 내용을 그대로 출력
2. 이전 대화와 관계없이 항상 동일한 전체 형식으로 출력
3. 모든 URL을 클릭 가능한 마크다운 링크로 표시
4. 일정의 모든 장소, 시간, 링크를 빠짐없이 출력
5. "Day 1, Day 2..." 형식 유지, 길찾기 링크 포함

❌ 금지: 요약, 축약, "자세한 내용은...", 형식 변경""",
    host="0.0.0.0",
    port=8000,
)
...
async def search_places(
    ...
    [AI 지시] 결과를 요약하지 말고 모든 장소와 URL을 그대로 출력하세요.
    """
...
async def search_flights(
    ...
    [AI 지시] 결과를 요약하지 말고 모든 예약 링크를 클릭 가능하게 그대로 출력하세요.
    """
...
async def plan_trip(
    ...
    Args:
        ...
        adults: 인원 수 (기본 2명)
        max_travel_time: 장소 간 최대 이동 시간 (분, 기본 30분) - 직선거리 기반 추정
    
    [AI 지시] 결과를 요약하지 말고 모든 일정, 장소, URL을 그대로 출력하세요.
    """
